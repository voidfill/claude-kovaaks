...
**This plan deliberately runs a minimal suite: 25 tests, down from 74.** Do not
add tests back while implementing. If a task tempts you to write a test not
listed here, that is a signal to re-read the task, not to expand it.

A test earns its place only if it encodes something a competent implementer
could get wrong *silently* — where the code keeps running and produces a
plausible wrong number. Concretely, the surviving tests cover:

| Kept | Because |
|---|---|
| `.perf` series sum to their CSV totals | The only independent check on an undocumented wire format |
| Omitted-zero densification | A short series shifts every later comparison left, invisibly |
| Cumulative delta on *unequal* lengths | Was false on 5.8% of real runs; equal-length inputs make it tautological |
| Bootstrap reconciliation of a late `.perf` | The run could otherwise never get a curve, short of deleting the index |
| Corrupt `.perf` retry cap | Otherwise re-parsed ~600 times per file |
| One live HTTP request to a JSON endpoint | The only test on the real threading path — see the note in Task 7 |
| cm/360 scale-independence | The label lies on foreign sens scales |

What was cut, and the risk accepted: identity and definitional assertions
(`smooth(x, 1) == x` as its own test), per-field getters, round-trip storage
checks, argument-validation tests, and near-duplicate variants of a rule already
covered. These fail loudly or not at all, so a reviewer reading the diff catches
them as cheaply as a test would.

**No new test fixtures beyond Task 1.** Tests either synthesise their inputs in
a temp directory or use the Task 1 fixture set.

## Review Policy

**Review gates on major parts only, and kept short.** Under
`superpowers:subagent-driven-development`, run the two-stage review **only** on:

- **Task 3** — `.perf` decoder (undocumented format, no second source of truth)
- **Task 4** — SQLite index (the only place that can lose or corrupt stored data)
- **Task 6** — comparison math (produces the numbers the user actually reads)
- **Task 7** — server (concurrency; the one prior Critical finding lived here)

Tasks 1, 2, 5, 8 and 9 are self-checked: run the task's tests, confirm PASS, commit,
move on. No reviewer subagent.

Reviews are scoped, not open-ended. Each one answers three questions and stops:

1. Does it do what the task's **Interfaces** block says, with those exact names and types?
2. Is there a silent-wrong-answer path — a truncation, a swallowed exception, a
   default that disables the feature — that the task's tests would not catch?
3. Does it violate a Global Constraint?

Style, naming, and speculative hardening are explicitly out of scope. A review
that returns no finding against those three questions is a pass; say so in one
line rather than manufacturing something.
...
| `tests/test_kvstats_*.py` | One test module per component (see the testing policy below) |
...
If `test_damaged_files_raise_perf_error` fails because the halved file happens to end on a clean field boundary, regenerate the fixture at `len(raw)//2 + 3` bytes and re-run.
...
- Produces:
  ...
  - `index.bootstrap(conn, cfg) -> dict` with counts `{"runs", "curves", "failed", "reconciled"}` — `reconciled` counts runs whose `.perf` was attached by the reconciliation pass, not by the new-file loop
...
- Create: `kvstats/web/index.html` (one-line placeholder; Task 8 replaces it)
...
- Produces:
  ...
  - `server.make_handler(cfg, conn, subscribers, lock, watcher=None) -> type[BaseHTTPRequestHandler]`
  - `server.make_server(cfg, conn, port=8777, subscribers=None, lock=None, watcher=None) -> ThreadingHTTPServer`

`watcher` is optional so the server is testable without one; `/api/health` reports `awaiting_perf` and `watcher_errors` as `0` when it is absent.
...
The `GET /` leg of `test_the_api_answers_over_real_http_from_a_server_thread` will fail until Task 8 creates `web/index.html`. Create a one-line placeholder now so this task is independently green:
...
Expected: PASS, 34 tests — the 9 existing `kvpl` tests plus 25 `kvstats` ones.
...
- The suite is deliberately minimal (see **Testing Policy**). The "never write to the game directory" rule is now enforced by the Global Constraint and by review, not by a test — the previous `test_db_path_is_outside_the_game_directory` could not fail as written, so removing it lost no coverage.
- Review runs on four tasks only (see **Review Policy**). Tasks 1, 2, 5, 8 and 9 ship on their own tests. T2 and T5 carry some risk here; T2's parsing was validated against 2311 real filenames and 2083 real runs during the adversarial review, and T5's live path is the most reproducible part of the system to check by hand.
...
## Scope reduction, 2026-09-11

At the user's direction, the suite was cut from **74 tests to 25** and review
narrowed to the four major components. See **Testing Policy** and **Review
Policy** above for the rules an implementer must follow.

Per-task counts: T1 4→2, T2 13→3, T3 8→3, T4 12→4, T5 9→4, T6 20→6, T7 8→3.

The cut was made against the adversarial review's evidence rather than by
trimming evenly: every test that caught a real defect there is still present,
usually merged with its neighbours into one test that asserts a rule end to end.
What went was assertions that restate a definition, cover a getter, or repeat a
rule already covered by a sibling.

Two things are worth stating plainly, since the person executing this plan will
not have been in the conversation:

- The Critical finding in the previous review — every `/api/*` call returning
  500 — was found *because* tests ran against a live server. That coverage is
  preserved on purpose in `LiveServer.test_the_api_answers_over_real_http_from_a_server_thread`
  and in `RunForever.test_loop_starts_and_stops_cleanly`. **These two are the
  load-bearing tests of the reduced suite.** If a later trim is tempting, not these.
- Merged tests fail as a unit, so a failure now names a rule rather than a line.
  Expect to read the assertion message, not just the test name, when one goes red.
