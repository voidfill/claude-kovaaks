...
- **Every `sqlite3.connect` must pass `check_same_thread=False`.** The watcher and each HTTP handler run on their own threads; the default raises `ProgrammingError` and silently disables the entire runtime.
...
- Produces:
  ...
  - `paths.Config` (plain `__slots__` class): `root: str`, `stats_dir: str`, `perf_dir: str`, `db_path: str`, `playlist_in_progress: str`
...
**Interfaces:**
- Consumes: nothing (pure; deliberately does not import `paths`)
- Produces:
...
class Cm360(unittest.TestCase):
    ...
    def test_the_label_is_ignored_so_foreign_scales_still_resolve(self):
        """The FINALS 33 @ 400 DPI and Overwatch 5 @ 400 DPI are one sensitivity.

        Their `Horiz Sens` labels differ by 6.6x because they are in different
        scales. cm/360 is derived from DPI and increment only, so the label
        cannot influence it -- which is the entire reason the formula exists.
        """
        self.assertAlmostEqual(statscsv.cm360(400, 0.471429), 69.27, places=1)

    def test_the_same_sensitivity_at_two_dpis_gives_the_same_cm360(self):
        # 30 cm/360 is increment 0.272143 at 1600 DPI, 1.088572 at 400 DPI.
        self.assertAlmostEqual(statscsv.cm360(1600, 0.272143), 30.0, places=2)
...
class Reconciliation(IndexBase):
    def test_a_perf_that_arrives_after_the_csv_is_picked_up_on_a_later_bootstrap(self):
        ...
        would permanently bake in a curveless run.
        """
...
Expected on the reference install: `{'runs': 2311, 'curves': 1993, 'reconciled': 0, 'failed': 0}`, about **4 s**, DB around 6 MB.

Two failure signals, both worth stopping for:

- **Counts materially lower than 2311/1993** — the parser is dropping runs.
- **Time above ~15 s** — almost certainly a stray per-file `conn.commit()`. Committing once per file costs 4304 fsyncs and turns a 4 s scan into 68 s. Pure parse time is only ~3 s, so anything much above that is I/O barrier, not work.
...
- Produces:
  - `watch.Watcher(cfg, conn, on_run=None, tick=0.1, full_scan_every=10.0, perf_deadline=60.0)`
  - `Watcher.poll_once() -> list[int]` — run ids newly indexed by this call
  - `Watcher.run_forever(stop_event)` — loops until the event is set
  - `Watcher.stats -> dict` with `{"scans", "ticks", "errors", "awaiting_perf"}`. `stats["awaiting_perf"]` is the live `_awaiting` dict, aliased not copied — it is only ever mutated in place, and the tests assert against it directly.
...
- Produces:
  ...
  - `compare.pad(a: list, b: list) -> tuple[list, list]`
  - `compare.compare_until(a: list, b: list) -> int`
...
```python
{
  ...
  "recent":      {"n": int, "mean_score": float,
                  "curve": list[dict] | None},   # a LIST of curves, one per run
  "candidates":  int,
}
```

`recent.curve` is a list because `server.build_run_payload` turns it into a per-bucket band across those runs. `pb.curve` is a single curve dict.
...
class PureMath(unittest.TestCase):
    ...
    def test_cumulative_delta_survives_unequal_lengths(self):
        """The invariant, tested where it actually breaks.
    ...
    def test_cumulative_delta_is_symmetric_about_which_run_is_longer(self):
        got = compare.cumulative_delta([1.0, 1.0], [0.5, 0.5, 0.5, 0.5])
        self.assertEqual(len(got), 4)
        self.assertAlmostEqual(got[-1], 2.0 - 2.0, places=6)

    def test_compare_until_marks_where_only_one_run_has_data(self):
    ...
    def test_band_collapses_to_the_mean_under_three_curves(self):
        ...
        got = compare.band([[0.0, 10.0], [10.0, 20.0]])
...
| `GET /api/scenarios` | name, run count, PB, last played, recent form |
| `GET /api/session/today` | today's runs with deltas |
| `GET /api/health` | `runs`, `curves`, `failed`, `scenarios`, `awaiting_perf`, `watcher_errors`, `suspect_fov` |
...
class RunPayload(ServerBase):
    ...
    def test_smoothing_changes_the_curve_but_not_the_delta(self):
        """The delta is raw and score-based, so smoothing must not touch it."""
    ...
    def test_the_delta_is_score_units_whatever_metric_is_selected(self):
        """A running sum of per-second accuracy differences is meaningless."""
        run_id = self.run_id_with_curve()
        by_score = server.build_run_payload(self.conn, run_id, metric="score")
        by_accuracy = server.build_run_payload(self.conn, run_id, metric="accuracy")
        self.assertNotEqual(by_score["curve"], by_accuracy["curve"])
        self.assertEqual(by_score["cumulative_delta"],
                         by_accuracy["cumulative_delta"])
...
The first launch indexes your whole history into a
SQLite cache outside the game directory (about 4 s for ~4300 files). After that it
only reads new files. The cache is disposable — delete it and it rebuilds.
...
The same block says `tests/  four tests: encodings, byte-exact writes, the gate, cloning`. That count is already stale and is now well out of date — replace that line with:
...
- `test_db_path_is_outside_the_game_directory` (T1) cannot fail as written — `default_db_path()` is under `%LOCALAPPDATA%`, unrelated to the fixture root. It is kept as an intent guard, not as a real test.

## Adversarial review, 2026-09-10

An independent reviewer extracted every code block in this plan into real files, built the Task 1 fixtures from the live install, and ran the suite. Findings applied:

| Severity | Finding | Fix |
|---|---|---|
| Critical | `sqlite3.connect` defaulted to `check_same_thread=True`, so the watcher thread and every HTTP handler raised `ProgrammingError`. Every `/api/*` call would 500 and nothing would ever index; only `GET /` worked, which is what let two tasks claim PASS. | `check_same_thread=False` |
| High | A run indexed before its `.perf` was written could **never** get a curve — `bootstrap` only considers unseen CSVs, and the watcher only tracks files it saw arrive. Recoverable only by deleting the index. | reconciliation pass in `bootstrap` |
| High | The spec's headline invariant — cumulative delta ends at the score difference — was **false on 5.8% of real runs** (`align` truncated to the shorter curve), the headline `% vs PB` used a different baseline than the chart, and a non-score metric produced a meaningless running sum. The only test asserted a definition and could not fail. | pad instead of truncate; `delta_baseline` in the payload; delta pinned to score units; test rebuilt on unequal lengths |
| Med-high | A corrupt `.perf` was re-parsed and re-committed on every 100 ms tick for 60 s (~600 times). `MAX_TRIES` capped the counter, not the retries. | drop from `_awaiting` at `MAX_TRIES` |
| Medium | Bootstrap took **68 s, not the 20-25 s stated** — `commit()` per file is 4304 fsyncs against 3.2 s of actual parsing. | batch commits; corrected the stated budget and its failure signal |
| Medium | `run_forever`'s bare `except Exception: pass` is what made the critical bug invisible. | log the traceback, count it in `/api/health` |
| Medium | `slice(0, n)` truncates a long baseline but never pads a short one; ~6% of runs rendered a silently shortened overlay. | `fit()` pads with `null`; tail marked |
| Medium | Un-debounced `resize` refetched the API and rebuilt both charts ~60x/s. | debounce 150 ms, `setSize` instead of refetch |
| Low | `startswith(WEB)` admitted sibling directories named `web*`; five interface declarations disagreed with the code; a broken markdown fence truncated the README section; two test counts were wrong. | all corrected |

Also closed, from spec requirements no task had delivered: tail marking, the "no band under 3 candidates" rule, cm/360 stored to 2 dp, junk FOV surfaced in `/api/health`, recent form in `/api/scenarios`, and a port fallback that actually fails to bind on Windows.

**Verified sound and left alone:** the `.perf` decoder (2001 files, zero errors; all 7 series cross-summed against 1993 CSVs with zero mismatches), the cm/360 formula (2083 runs, zero mismatches), the filename regex (2311 filenames, zero failures, including the three scenarios containing ` - `), the whole schema and the `INSERT OR IGNORE` + `rowcount` guard, and the concurrency design itself once the connection flag is set — a live end-to-end run delivered the SSE event 42 ms after the CSV landed.
